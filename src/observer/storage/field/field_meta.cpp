...
FieldMeta::FieldMeta(
    const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int field_id, bool nullable, int vector_dim)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, field_id, nullable, vector_dim);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(
    const char *name, AttrType attr_type, int attr_offset, 
    int attr_len, bool visible, int field_id, bool nullable, int vector_dim)
{
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  const Json::Value &vector_dim_value = json_value[FIELD_VECTOR_DIM];
  ...
  int         vector_dim = vector_dim_value.asInt();
