...
RC DiskBufferPool::append_vector_data(int64_t &offset, int64_t length, const float *data)
{
  RC rc = RC::SUCCESS;
...
RC DiskBufferPool::get_vector_data(int64_t offset, int64_t length, float *data)
{
  if (lseek(file_desc_, offset, SEEK_SET) == -1) {
    LOG_ERROR("Failed to lseek %s at offset %d :%s.", file_name_.c_str(), offset, strerror(errno));
    return RC::IOERR_SEEK;
