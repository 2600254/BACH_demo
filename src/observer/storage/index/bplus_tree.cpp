    ...
    int leaf_max_size /* = -1 */)
{
  ...
  rc = this->create(log_handler, *bp, file_name, unique, field_ids, fields, internal_max_size, leaf_max_size);
  if (OB_FAIL(rc)) {
...
RC BplusTreeHandler::create(LogHandler &log_handler, DiskBufferPool &buffer_pool, const char *file_name,
    const bool unique, const std::vector<int> &field_ids, const std::vector<const FieldMeta *> &fields,
    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */)
{
  ...
  file_header->attr_num          = field_ids.size();
