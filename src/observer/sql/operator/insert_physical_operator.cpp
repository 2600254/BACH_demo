...
InsertPhysicalOperator::InsertPhysicalOperator(BaseTable *base_table, vector<Value> &&values)
...
RC InsertPhysicalOperator::insert_into_view(Trx *trx)
{
  ...
  for (auto col_of_table : view_col_idx) {
    ...
    const std::vector<int> &field_idx_in_one_table = field_idx_in_tables[table];
      ...
      // 补齐一行数据
    std::vector<Value> fixed_row_value(table->table_meta().field_num() - table->table_meta().sys_field_num(), {AttrType::NULLS, nullptr, 0});
    for (size_t i = 0; i < field_idx_in_one_table.size(); i++) {
      fixed_row_value[field_idx_in_one_table[i]] = values_[col_of_table.second[i]];
    }
